...
best.rsids <- c("rs1000987", "rs73554189", "rs35491396", "rs7840424")
mtx.geno <- etx$getGenoMatrixByRSID(best.rsids)
mtx.geno.numeric <- matrix(0, nrow=nrow(mtx.geno), ncol=ncol(mtx.geno))
mtx.geno.numeric[which(mtx.geno == "0/1")] <- 1
mtx.geno.numeric[which(mtx.geno == "1/1")] <- 2
colnames(mtx.geno.numeric) <- colnames(mtx.geno)
rownames(mtx.geno.numeric) <- rownames(mtx.geno)
heatmap.2(mtx.geno.numeric, trace="none", col=c("black", "grey", "red"),
          margins=c(5,15))

table(mtx.geno)
table(mtx.geno.numeric)
heatmap(mtx.geno.numeric)
dim(mtx.geno.numeric)
plot(hclust(dist(mtx.geno.numeric)))
